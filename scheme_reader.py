...
def read_tail(src):
    ...
    try:
        if src.current() is None:
            ...
            src.remove_front()
            srcn = scheme_read(src)
            
            if read_tail(src) == nil: #if there is nothing after after the dot
                return srcn    #then we return what is after the dot

            else:
                raise SyntaxError() #if there is something after after the dot
                #then it is going to be an error                
