...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        operator = scheme_eval(first, env) #operator will be our + sign
...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        return scheme_apply(self,operands.map(lambda x: scheme_eval(x ,env,False)), env) #lambda is the funciton
...
def do_and_form(expressions, env):
    ...
    if len(expressions) == 1:
       return scheme_eval(expressions.first, env,True)
    varf = scheme_eval(expressions.first,env,False)
    

    if scheme_falsep(varf):
          return varf
    else:
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if clause.first == 'else':
            ...
            test = scheme_eval(clause.first, env,False)
...
def do_define_macro(expressions, env):
    ...
    if scheme_symbolp(tar.first) and isinstance(tar, Pair):
        parameters = tar.second
        check_formals(parameters)
        body = expressions.second

        macrofunc = MacroProcedure(parameters, body, env)
        env.define(tar.first, macrofunc)
        return tar.first
    else:
        raise SchemeError
